...
%Hier Fehlt noch ne zweite Überschrift
...
\paragraph{Dynamic light scattering}\mbox{}\\
...
Where N is the number of colloids in the system and b($\vec{q}=0$) is the normalization of the form factor.
...
\end{equation} 
The hydrodynamic radius $a_h$ is the effective dimension how a particle in a medium is seen for other particles. It consists of the usual radius, that we can measure through the form factor by scattering, and an additional layer of particles around. It's comparable to adsorbed particles onto the colloids surface. That means it depends on the colloids surface and the size of particles in the medium. We can obtain it by the autocorrelation function (\ref{eq:g2}) with the relation: 
...
Now looking at Figure \ref{FormA}. The first fit includes every point and does not look like the function describing our data. Although its result is only off by a factor of 2 compared to the given particle radius of $35nm$. The partial fits on the other hand are more divergent in their radius. As 
...
The result looks pretty much the same. The only thing that really changed was the total intensity which reduced from around 140 to 60. The are neglecting this change by a independent constant and thereby we end up with nearly the same radius compared to sample A. 
