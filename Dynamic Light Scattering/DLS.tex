...
%noch n fehler an die 62 nm dran, als mittelwert der fitfehler a
...
\begin{table}[!htbp]
	...
	\label{tab:adata}
\end{table}
%Schreiben das die daten gut sind,bzw alle gleich
...
%Vllt bisden sich hier kleine klumpen oder irgendwas anderes,dass uns unseren fit kaputt macht, aber keine ahnung... Aber wir können es nicht fitten....bzw wird es nicht besser wenn wir es fitten. 
%Theorie: Es geht kaputt weil mit einern höheren konzentration der die kristallisation beginne
\subsection{C: Small particles in fluid phase, high concentration}
Comparing the autocorrelations from sample A with figure \ref{C} the fit is no longer fitting that well. The data is no longer in the shape of an usual exponential function like it was for the first two samples A and B. For a poly-disperse the shape would stay the same and would only change in the correlation time $\tau$. The larger the colloid is the longer the later momentum is correlated to the starting one. In this case the curve would only wander a bit to the right. But the data shows something like a convolution in the dropping part which doesn't make any sense. This could mean that there are other things affecting the correlation which we don't know about.

We assume that this discrepancy to the fit is an indicator that we have to take care about effects caused by poly-disperses. It could be that the colloids start to form structures (crystallise) but the concentration is not high enough to form stable ones for a long time. 
...
%Noch schriebne warum wir wie vorgegangen sind und noch erklären welcher graph welcher ist also der grüne für die kleinen teilchen und der orangene für die kristallinen poarticles
